...
void				 		app_gatt_connect(wiced_bt_gatt_connection_status_t *p_conn);
		...
		case BTM_BLE_ADVERT_STATE_CHANGED_EVT:					// Advertising State Change
			...
			switch( p_event_data->ble_advert_state_changed )
			{
				case BTM_BLE_ADVERT_OFF:
					wiced_hal_pwm_change_values( PWM0, connection_id ? PWM_ALWAYS_OFF : PWM_ALWAYS_ON, PWM_INIT );
					break;
...
/*******************************************************************************
* Function Name: demo_app_init()
********************************************************************************/
void demo_app_init()
{
	/*Configure the GPIO()LED2 and PWM(LED1)*/
...
/*******************************************************************************
* Function Name: app_gatt_get_value( wiced_bt_gatt_attribute_request_t *p_attr )
********************************************************************************/
wiced_bt_gatt_status_t app_gatt_get_value( wiced_bt_gatt_attribute_request_t *p_attr )
{
        ...
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if (app_gatt_db_ext_attr_tbl[i].cur_len <= *p_len)
            {
                ...
                *p_len = app_gatt_db_ext_attr_tbl[i].cur_len;
                ...
                switch ( attr_handle )
                {
					case HDLC_DEMO_COUNTER_VALUE:
						WICED_BT_TRACE( "Current counter value is 0x%02x\r\n", app_demo_counter[0]);
						break;

					case HDLC_DEMO_LED_VALUE:
				WICED_BT_TRACE( "LED is %s\r\n", app_demo_led[0] ? "ON" : "OFF" );
						break;
                }
            }
            else
...
/*******************************************************************************
* Function Name: app_gatt_set_value(wiced_bt_gatt_attribute_request_t *p_attr )
********************************************************************************/
wiced_bt_gatt_status_t app_gatt_set_value( wiced_bt_gatt_attribute_request_t *p_attr )
{
	uint16_t attr_handle = 	p_attr->data.handle;
    ...
    for (i = 0; i < app_gatt_db_ext_attr_tbl_size; i++)
    {
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if (validLen)
            {
                ...
                {
					case HDLC_DEMO_COUNTER_VALUE:
						WICED_BT_TRACE( "Counter value set to 0x%02x\r\n", app_demo_counter[0]);
						break;

					case HDLD_DEMO_COUNTER_CLIENT_CHAR_CONFIG:
						WICED_BT_TRACE( "Setting notify (0x%02x, 0x%02x)\r\n", p_val[0], p_val[1] );
						break;

					case HDLC_DEMO_LED_VALUE:
						wiced_hal_gpio_set_pin_output( WICED_GPIO_PIN_LED_2, app_demo_led[0] == 0 );
...
/*******************************************************************************
*Description: This function handles the GATT connection

*Function Name: app_gatt_connect(wiced_bt_gatt_connection_status_t *p_conn)
********************************************************************************/
void app_gatt_connect(wiced_bt_gatt_connection_status_t *p_conn)
{
	if( p_conn->connected )
...
/*******************************************************************************
*Description: This function handles button interrupts and start notification
...
void button_cback( void *data, uint8_t port_pin )
{
	...
	if( connection_id )
	{
		if( app_demo_counter_client_char_config[0] & GATT_CLIENT_CONFIG_NOTIFICATION )
		{
			WICED_BT_TRACE( "Notifying counter change (%d)\r\n", app_demo_counter[0] );
			wiced_bt_gatt_send_notification(connection_id,
											HDLC_DEMO_COUNTER_VALUE,
											app_demo_counter_len,
											app_demo_counter );
		}
    }
