...
void DHT_Task(void* pvParameters)
{
	...
	for(;;)
	{
		...
		if(DHT_reading.result_code == SUCCESS)
		{
			...
			/* *************************************************************
			 * If the sensor values are valid, overwrite the queue with the
			 * updated readings.
			 * *************************************************************/
			xQueueOverwrite(print_queue, &DHT_reading);
		}
		else
		{
			...
			/* *************************************************************
			 * If the sensor values are not valid, overwrite the queue with
			 * the error code and the previous value. Print_Task reads the
			 * error value and decides the course of action. The other tasks
			 * ignore this value.
			 * *************************************************************/
			xQueueOverwrite(print_queue, &DHT_reading);
		}
		...
		/* *****************************************************************
		 * Set the event bits corresponding to all tasks so that the tasks
		 * are notified when the queue is updated. Each task clears its
		 * respective event bit after reading the sensor values from the
		 * queue.
		 * *****************************************************************/
		xEventGroupSetBits(dht_reading_notify_event_group,
				(PRINT_TASK_NOTIFY_EVENTBIT |
